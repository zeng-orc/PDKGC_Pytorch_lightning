...
class DisenLayer(MessagePassing):
    def __init__(self, edge_index, edge_type, in_channels, out_channels, num_rels, act=lambda x: x, params=None, head_num=1):
        ...
        self.loop_index = torch.stack([torch.arange(self.p.n_ent), torch.arange(self.p.n_ent)]).to(self.device)
        self.loop_type = torch.full((self.p.n_ent,), 2 * self.num_rels, dtype=torch.long).to(self.device)
        n_ent = self.p.n_ent
    ...
    def _get_attention(self, edge_index_i, edge_index_j, x_i, x_j, rel_embed, rel_weight, mes_xj):
        if self.p.att_mode == 'learn':
            ...
            alpha = softmax(alpha, edge_index_i, num_nodes=self.p.n_ent)
